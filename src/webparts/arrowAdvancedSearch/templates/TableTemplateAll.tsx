...
export interface ITableTemplateAll extends IArrowSearchWebPartProps {
	context?: IWebPartContext;
	results: any[];
}

export default class TableTemplateAll extends React.Component<ITableTemplateAll, {}> {
...
<a href={result.Path}><img  className={styles.templateListImg}  src={`${this.iconUrl}${result.Fileextension !== null && this.unknown.indexOf(result.Fileextension) === -1 ? result.Fileextension : 'code'}.png`} alt="File extension"/></a>
							...
							<div className={styles.templateListDescription} >{result.HitHighlightedSummary} </div>
