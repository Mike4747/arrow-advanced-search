...
appDispatcher.register((action) => {
	switch (action.actionType) {
		case searchActionIDs.SEARCH_GET:
			...
			if (searchStore.isEmptyString(action.searchQuery)) {
			...
			url += !searchStore.isEmptyString(action.searchQuery) ?
				`'${tokenHelper.replaceTokens(action.searchQuery, action.context)}` : "'*'";

			// Check if a refiner query is provided
			url += !searchStore.isEmptyString(action.query) ?
				` ${tokenHelper.replaceTokens(action.query, action.context)}'` : "'";
...
// Add the refinement (document type)
url += "";
url += !searchStore.isEmptyString(action.refinementFilters) ? action.refinementFilters : "";
			// Add the rowlimit
			url += "&rowlimit=";
